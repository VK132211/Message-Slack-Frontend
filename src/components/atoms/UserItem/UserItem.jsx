...
const userItemVariants = cva("flex items-center gap-1.5 justify-start font-normal h-7 px-4 mt-2 text-sm", {
  variants: {
    variant: {
    ...
    <Button className={cn(userItemVariants({ variant }))} variant="transparent" size="sm" asChild>
      <Link to={`/workspaces/${workspace?._id}/members/${id}`}>
        <Avatar>
          ...
          <AvatarFallback className="rounded-md bg-sky-500 text-white">{label.charAt(0).toUpperCase()}</AvatarFallback>
        </Avatar>
